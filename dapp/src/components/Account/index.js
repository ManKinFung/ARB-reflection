...
export const Account = (props) => {
  ...
  const { symbol, rewardTokenSymbol, price: tokenPrice, rewardTokenPrice, lastRebasedTime: lastRebased, initRebaseTime: initRebasedOrg,
  ...
  useEffect(() => {
    setTotalRewardsUSD(BN(rewardTokenPrice).times(BN(totalRewards)).toString())
  }, [totalRewards, rewardTokenPrice])

  useEffect(() => {
    setRewardDistributedUSD(BN(rewardTokenPrice).times(BN(rewardDistributed)).toString())
  }, [rewardDistributed, rewardTokenPrice])

  useEffect(() => {
    setRewardedAmountUSD(BN(rewardTokenPrice).times(BN(rewardedAmount)).toString())
  }, [rewardedAmount, rewardTokenPrice])

  useEffect(() => {
    setRewardAmountUSD(BN(rewardTokenPrice).times(BN(rewardAmount)).toString())
    ...
    <AccountContainer>
      <div className='top-frame'>
        ...
        <div className='reflection-frame'>
          <div className='reflection-caption'>Reflection In {rewardTokenSymbol}</div>
        ...
        <div className='summary-frame-col info-frame'>
          ...
          <AccountItemInfo2 label='Total Rewards' detail={`$${stringFormat(cutDecimal(totalRewardsUSD, 6))}`} />
          <AccountItemInfo2 label='Distributed Rewards' detail={`$${stringFormat(cutDecimal(rewardDistributedUSD, 6))}`} />
          ...
          <AccountItemInfo2 label='Rewarded so far' detail={`$${stringFormat(cutDecimal(rewardedAmountUSD, 6))}`} />
          <AccountItemInfo2 label='To be rewarded' detail={`$${stringFormat(cutDecimal(rewardAmountUSD, 6))}`} />
        </div>
      </div>
    </AccountContainer>
  )
