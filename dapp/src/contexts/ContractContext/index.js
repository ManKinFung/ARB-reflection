...
import {
    updateAddress, updateName, updateSymbol, updateDecimals, updateRewardToken, updateRewardTokenName, updateRewardTokenSymbol, updateRewardTokenDecimals, updateRewardTokenPrice, updateRewardTokenPath, updateOwner, updatePrice, updateSupply,
...
export const ContractProvider = (props) => {
    ...
    const { treasuryAddress, insuranceReceiverAddress, firePitAddress, rewardToken, rewardTokenPath } = useSelector(state => state.token)
    ...
    const getRewardToken = useCallback(async (token) => {
        const web3 = web3Provider
        assertValidAddress(token)
        const tokenContract = new web3.eth.Contract(Nova_abi.abi, token)
        return await tokenContract.methods.feeToken().call()
    }, [web3Provider, assertValidAddress])

    const getRewardTokenPath = useCallback(async (token) => {
        const web3 = web3Provider
        assertValidAddress(token)
        const tokenContract = new web3.eth.Contract(Nova_abi.abi, token)
        const rewardTokenPath = [];
        try {
            let index = 0;
            while(true) {
                const _token = await tokenContract.methods.feeTokenPath(index).call();
                rewardTokenPath.push(_token);
                if(rewardToken != "" && _token === rewardToken ) {
                    break;
                }
                index++;
            }
    ...
    const rewardTokenPriceToUSDC = useCallback(async (amount) => {
        const web3 = web3Provider;
        const tokenContract = new web3.eth.Contract(Nova_abi.abi, ADDRESS[chainId].token)
        let routerAddress = await tokenContract.methods.router().call()
        const routerContract = new web3.eth.Contract(Router_abi.abi, routerAddress);
        ...
        let realVal = await D2A(rewardToken, amount)
        let _newPath = [...rewardTokenPath];
        _newPath.reverse();
        let t2 = await routerContract.methods.getAmountsOut(realVal.toString(), [..._newPath, ADDRESS[chainId].usdc]).call()
        return await A2D(ADDRESS[chainId].usdc, t2[t2.length - 1])
    }, [D2A, A2D, web3Provider, chainId])
    ...
    useEffect(() => {
        ...
        getRewardToken(ADDRESS[chainId].token)
            .then(r => {
                if (ac.signal.aborted === false) {
                    dispatch(updateRewardToken(r))
                }
            })
            .catch(err => {
                console.log(err)
        ...
        if(rewardToken)  {
            getRewardTokenPath(ADDRESS[chainId].token)
                .then(r => {
                    if (ac.signal.aborted === false) {
                        dispatch(updateRewardTokenPath(r))
                    }
                })
                .catch(err => {
                    console.log(err)
                })
            getTokenName(rewardToken)
                .then(r => {
                    if (ac.signal.aborted === false) {
                        dispatch(updateRewardTokenName(r))
                    }
                })
                .catch(err => {
                    console.log(err)
                })
            getTokenSymbol(rewardToken)
                .then(r => {
                    if (ac.signal.aborted === false) {
                        dispatch(updateRewardTokenSymbol(r))
                    }
                })
                .catch(err => {
                    console.log(err)
            ...
            getTokenDecimals(rewardToken)
                .then(r => {
                    if (ac.signal.aborted === false) {
                        dispatch(updateRewardTokenDecimals(r))
                    }
                })
                .catch(err => {
                    console.log(err)
        ...
        // console.log(">>> rewardTokenPath=", rewardTokenPath, "flag=", rewardTokenPath.length > 0)
        if (rewardTokenPath.length > 0) {
            console.log(">>> rewardTokenPriceToUSDC rewardTokenPath=", rewardTokenPath);
            rewardTokenPriceToUSDC(1)
                .then(r => {
                    if (ac.signal.aborted === false) {
                        // console.log(">>> rewardTokenPrice:", r.toString())
                        dispatch(updateRewardTokenPrice(r.toString()))
                    }
                })
                .catch(err => {
                    console.log(err)
        ...
        return () => ac.abort()
    }, [dispatch, chainId, getRewardToken, getRewardTokenPath, getTokenName, getTokenSymbol, getTokenDecimals, getOwner,
        rewardTokenPriceToUSDC, tokenPriceToUSDC, getCirculatingSupply, lastRebasedTime, firePit, treasuryReceiver,
    ...
    useEffect(() => {
        ...
        return () => ac.abort()
    }, [reloadCounter, dispatch, chainId, treasuryAddress, insuranceReceiverAddress, firePitAddress, rewardToken, rewardTokenPath, wethBalanceByUSDC, balanceOf])
    ...
    return (
        <ContractContext.Provider value={{
            ...
            getRewardToken, getTokenName, getTokenSymbol, getTokenSupply, getOwner, isAddressFormat, buildEncodedData,
            verifyContract, rewardTokenPriceToUSDC, tokenPriceToUSDC, getCirculatingSupply, lastRebasedTime, firePit, treasuryReceiver,
