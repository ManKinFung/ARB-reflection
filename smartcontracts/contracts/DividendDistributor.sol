...
contract DividendDistributor is IDividendDistributor, Ownable
{
    ...
    function deposit(uint256 amount) external payable override onlyToken {
        require(feeToken != address(0), "FeeToken is not set yet");
        require(totalShares > 0, "total share is 0");
		
        uint256 beforeBalance = IERC20(feeToken).balanceOf(address(this));
        IERC20(feeToken).transferFrom(msg.sender, address(this), amount);
        uint256 _feeAmount = IERC20(feeToken).balanceOf(address(this)) - beforeBalance;
        totalDividends = totalDividends.add(_feeAmount);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(_feeAmount).div(totalShares));
    }
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            IERC20(feeToken).transfer(shareholder, amount);
        }
