...
const { addressZero, addressDead, bytes32Zero, maxUint256,
  ...
  DividendDistributor, Nova, ERC20Token, ERC20 } = require('./const')
...
const deploy_localhost = async (web3, deployer, accounts, specialAccounts) => {
    ...
    // console.log(factoryAddress, factoryInfo.imple)

    console.log("Pancake Factory Pair HASH:", await factoryContract.INIT_CODE_PAIR_HASH())

    const testERC20Info = await deployContract(deployer, "TestRewardToken", ERC20Token, "TestReward", "TRT");
    totalRet = syncDeployInfo(network, "TestRewardToken", testERC20Info, totalRet);

    // try {
    //   const tx = await factoryContract.createPair(wethAddr, testERC20Info.imple)
    //   console.log(tx)
    // } catch (err) {
    //   console.log(err)
    // }
    ...
    await tokenContract.setFeeTokenPath([wethAddr, testERC20Info.imple]);
    ...
    await reflectionContract.setFeeToken(testERC20Info.imple);
